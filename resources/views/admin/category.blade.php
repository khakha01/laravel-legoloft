         ...
         <form id="submitFormAdmin">
             ...
             <div class="buttonProductForm mt-3">
                 ...
                 <div class="m-0 p-0">
                     ...
                     <button type="button" class="btn btnF2" onclick="deleteAllCategory('{{ route('deleteCategory') }}')">
                                         ...
                                         <div class="form-check">
                                             <label class="switch_status_section pe-3">
                                                 <input type="checkbox" class="form-check-input-status-section"
                                                     ...
                                                     {{ $childCategory->status_section == 1 ? 'checked' : 0 }}>
                                                 <span class="slider_status_section"></span>
                                             </label>
                                             {{ $childCategory->status_section == 1 ? 'Bật' : 'Tắt' }}
                                         </div>
                                     </td>
                                     <td class="">
                                         <div class="actionAdminProduct m-0 py-3">
 ...
 @section('categoryAdminScript')
     ...
     <script>
         ...
         function updateEmployeeStatusSection(category_id, status_section, label) {
             $.ajax({
                 url: '{{ route('updateStatusSectionCategory', ':id') }}'.replace(':id', category_id),
                 type: 'PUT',
                 data: {
                     '_token': '{{ csrf_token() }}', //Việc gửi mã token này cùng với mỗi request giúp xác thực rằng request đó được gửi từ ứng dụng của bạn, chứ không phải từ một nguồn khác.
